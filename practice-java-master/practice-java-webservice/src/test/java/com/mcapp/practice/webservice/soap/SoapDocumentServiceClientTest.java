...
public class SoapDocumentServiceClientTest {
	@BeforeClass
	...
	@Test
	@PrepareForTest({ SoapDocumentServiceClient.class, Service.class })
	public void testMainWhenAllCasesReturnStaticValue() {
		...
		SoapDocumentService mockSoapDocumentService = mock(SoapDocumentService.class);
		...
		when(mockService.getPort(SoapDocumentService.class)).thenReturn(mockSoapDocumentService);
		when(mockSoapDocumentService.getServiceName()).thenReturn("Testing Service");

		// action
		try {
			SoapDocumentServiceClient.main(args);
		} catch (Exception e) {
			fail("Exception should not throw");
		}
